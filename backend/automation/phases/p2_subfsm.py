...
EMPTY_STREAK_LIMIT = 25           # 连续 N 帧无目标 + 不在大厅 → game_restart (死屏)
                                  # 12→25: YOLO 偶发漏检 (同画面别 inst 看得到, 这 inst 漏 12 帧)
                                  # 误判死屏导致测试模式反复 raise V3GameRestartRequested,
                                  # 资源未清理累积导致 backend 崩. 25 帧 (~50s) 才判死屏更稳.
