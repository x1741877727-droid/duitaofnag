...
# 死屏判定 — 改成"时间预算"+"phash 真卡住" 双条件 (替代纯 round 计数).
#
# 历史教训: 老代码 round_interval_s=0.5s + wait_seconds=0.3s, 单 round 实际 ~2s.
#   EMPTY_STREAK_LIMIT=12 → 24s 才触发, 跨越 loading 时间窗口刚刚好.
# 我后续把 round 加速到 ~1s/round, 12 round = 12s, loading 还没完就误判死屏.
#   单纯调高 EMPTY_STREAK_LIMIT 是"按 round 数算" — 不解决问题, 速度变了又得调.
# 正解: 用 time 而不是 round 数. 60s 真没动静 = 死屏, 不管你跑了多少 round.
DEAD_SCREEN_TIMEOUT_S = 60.0      # 连续 ≥ 此秒数无弹窗 + 不在大厅 + phash 卡住 → game_restart
PHASH_STUCK_THRESHOLD = 5         # phash 距离 ≤ 此 视为同一帧 (画面没变)
PHASH_STUCK_TIME_S = 20.0         # phash 卡住持续 ≥ 此秒数才视为真死屏
EMPTY_STREAK_LIMIT_LEGACY = 25    # 兼容: round 计数下限 (双条件之一, 防 phash 算错)
...
class P2SubFSM:
    ...
    async def step(self, ctx: RunContext) -> PhaseStep:
        ...
        # 5. 没目标 → 用时间和 phash 状态判 (不靠 round 计数)
        if action is None:
            ctx.empty_dets_streak += 1
            now = time.time()
            # 第一次进 no_target → 记起点
            if not getattr(ctx, "no_target_started_ts", 0):
                ctx.no_target_started_ts = now
            # 跟踪 phash 卡住的"起点时间", 不是 round 数
            ...
            if cur_ph and ctx.last_phash_int:
                ...
                if dist <= PHASH_STUCK_THRESHOLD:
                    if not getattr(ctx, "phash_stuck_started_ts", 0):
                        ctx.phash_stuck_started_ts = now
                else:
                    ctx.phash_stuck_started_ts = 0
            ...
            # 死屏判定 — 时间预算 (与 round 速度无关):
            #   - 距 no_target 起点超 DEAD_SCREEN_TIMEOUT_S (60s)
            #   - phash 卡住超 PHASH_STUCK_TIME_S (20s)  → 真画面冻住
            #   两者同时满足才认定死屏. loading 中 phash 一直在变, 不会触发.
            no_target_secs = now - (ctx.no_target_started_ts or now)
            stuck_started = getattr(ctx, "phash_stuck_started_ts", 0)
            stuck_secs = (now - stuck_started) if stuck_started else 0
            if (no_target_secs >= DEAD_SCREEN_TIMEOUT_S
                    and stuck_secs >= PHASH_STUCK_TIME_S):
                return PhaseStep(
                    PhaseResult.GAME_RESTART,
                    note=f"死屏: 无目标 {no_target_secs:.0f}s + phash 卡住 {stuck_secs:.0f}s",
                    outcome_hint="dead_screen",
                )
            return PhaseStep(
                PhaseResult.RETRY,
                note=f"无目标 ({no_target_secs:.0f}s, phash_stuck={stuck_secs:.0f}s)",
                outcome_hint="no_target",
            )

        # tap 成功 → 重置所有计数
        ctx.empty_dets_streak = 0
        ctx.no_target_started_ts = 0
        ctx.phash_stuck_started_ts = 0
