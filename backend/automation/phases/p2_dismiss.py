...
class P2DismissHandler(PhaseHandler):
    ...
    # 用 max_seconds=180s 作为真超时条件 — round 数受 perceive cold start 影响波动大
    # (R1 ~25s, 后续 ~200ms), 按 round 衡量不准. max_rounds 设大值仅作底层 loop 上限保护.
    max_rounds = 999
    max_seconds = 180.0
    round_interval_s = 0.25   # no_target 时复检间隔; YOLO 推理本身已耗 100-200ms, 不需要再额外 0.5s
